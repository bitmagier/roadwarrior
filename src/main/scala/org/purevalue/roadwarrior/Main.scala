...
import org.purevalue.roadwarrior.algorithms.{NearestFirstAlg, BruteForceAlg, Shortest2WayConnectionHeuristicAlg}
...
object Main {
  def main (args: Array[String]): Unit = {
    ...
    val cityMap = new CityMapGenerator ().gen (25)
    val algs: List[TravelingSalesmanAlg] = List (new NearestFirstAlg(cityMap), new Shortest2WayConnectionHeuristicAlg(cityMap)) //, new BruteForceAlg(cityMap))
